...
images/          two radiology jpgs  whole-image classification
...
is why the JSON is the event and the PDF is found from it rather than the other
way round. That path carries the `demo/` prefix; the rest mirrors the bucket
layout.

`info` is model output whose keys vary per pipeline — here a `metadata` object
holding scalars plus `priorities` / `continue` / `replace` / `add` arrays — so
the client walks it generically rather than assuming a vocabulary.

The PDFs are generated by the project's own ReportLab pipeline from public
NHANES 2009-2010 survey data, and carry no author or title metadata.

**`images/*.jpg`.** Plain rasters, loaded straight into the viewer — the event
path is the image itself, with no sidecar. Copied from the mock data the IVe and
CADA Plus apps already bundle, so they were already shipped publicly.
